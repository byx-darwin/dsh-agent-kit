...
export function renderLlmsFull(): string {
  ...
  return [
    ...
    '- 四个 Service 默认禁用、按需启用；业务包只 inject 用到的 Service，并把 @mc/dsh-agent-kit 声明为 peer 依赖。',
    ...
    '- npx @mc/dsh-agent-kit setup [--profile <名字>]：交互式配置，预览 cordis.patch.yml 的改动并确认后写入。',
    '- dsh Web「设置 → Agent Kit」：查看状态、启停 Service、修改配置、保存 TypeSafe Key；仅在 dsh Web 绑定 127.0.0.1 时可写。',
    '',
    `完整文档见 ${page('/docs/')}`,
    '',
  ].join('\n')
